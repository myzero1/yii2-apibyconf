...
namespace myzero1\apibyconf\components\rest;
...
class ApiHelper
{
    ...
    public static function addOptionsAction($urlRule)
    {
        ...
        if ($urlRule['controller'][0] == 'apibyconf_custom_rules') {
    ...
    public static function getApiConf($moduleId)
    {
        ...
        $confDataPathDefault = Yii::getAlias('@vendor/myzero1/yii2-apibyconf/src/components/conf/conf.json');
    ...
    public static function getApiUrlRules($moduleId)
    {
        ...
        $confDataPathDefault = Yii::getAlias('@vendor/myzero1/yii2-apibyconf/src/components/conf/apiUrlRules.php');
    ...
    /**
     * Get the module's name of apibyconf.
     *
     * 调用实例：Helper::
     *
     * @param   void
    ...
    public static function getRestModuleName()
    {
        foreach (\Yii::$app->modules as $key => $value) {
            if (!is_array($value)) {
                if (is_object($value)) {
                    if ('myzero1\apibyconf\Module' == $value::className()) {
                        return $key;
                    }
                } else {
                    if ('myzero1\apibyconf\Module' == $value) {
                        return $key;
                    }
                }
            }
    ...
    /**
     * Get the module's id of apibyconf.
     *
     * 调用实例：Helper::
     *
     * @param   void
     * @return  string
     **/
    public static function getapibyconfModuleId()
    {
        $moduleId = [];
        foreach (\Yii::$app->modules as $key => $value) {
            if (!is_array($value)) {
                if (is_object($value)) {
                    if (stripos($value::className(), "apibyconfModule") !== false) {
                        $moduleId[] = $key;
                    }
                }
            }
     ...
     * @return  string
     **/
    public static function getClassPath($className = 'myzero1\apibyconf\Module')
    {
    ...
    public static function getModuleClass($moduleId, $setDefault = false)
    {
        if (isset(Yii::$app->modules[$moduleId])) {
            ...
            if ($setDefault) {
                return sprintf('app\modules\%s\%s', $moduleId, 'apibyconfModule');
            } else {
